...
6. 在唯一的“根运动”选择器中选择：
   ...
   “动作空间”默认选择 `跟随目标初始布置`：目标人物提前移动、转向或放在父级 Empty 下时，动作的前后左右会跟随人物自身朝向；MMD `全ての親／センター` 等根控制骨的现有位置也会作为叠加基线保留。仅在复现 0.7 及更早版本时选择 `保持官方世界方向`。“肢端防穿模”默认开启，用于修正不同肩宽和臂长比例造成的双手额外收拢。
...
`Jaw`、眼睛和官方骨架的手部末端点不是完整 MMD 表情/手指链，默认不映射。表情、手指、精确武器接触、裙摆和头发放到 CORR、Shape Key 或物理流程处理。
...
该功能解决的是重定向额外产生的腕点收拢，不会凭空生成官方 30 骨骨架中不存在的手指动画，也不会把有意的握手、合掌或接触动作强行分开。需要逐指接触或严格抓握道具时，仍应在输出 Action 之后制作 CORR/手指关键帧。
